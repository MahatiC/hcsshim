...
func writeWindowsSignals(builder *strings.Builder, signals []string, indent string) {
	...
	writeLine(builder, `%s"windows_signals": %s,`, indent, array)
}

func writeWindowsUser(builder *strings.Builder, user string, indent string) {
	writeLine(builder, `%s"windows_user": "%s",`, indent, user)
}
...
func (p windowsContainerExecProcess) marshalRego() string {
	...
	return fmt.Sprintf(`{"command": %s, "windows_signals": %s}`, command, signals)
}
